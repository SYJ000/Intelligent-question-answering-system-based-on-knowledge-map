...
$('#info').keyup(function(e) {
    ...
    if ($('#info').val() == '') {
        $('#send').css({
            'background-image': 'url(/static/img/click.jpg)'
        });
    } else {
        $('#send').css({
            'background-image': 'url(/static/img/click.jpg)'
...
$('#send').click(function() {
    ...
    if (!pre_time || (pre_time && diff_time(d))) {
        var p = "<div><span style='color:white'>" + d.getHours() + ':' + (d.getMinutes().toString().length == 1?'0'+d.getMinutes():d.getMinutes()) + '</span></div>';
        pre_time = d;
        $('#chat').append(p);
    }
    // 获取当前问题信息
